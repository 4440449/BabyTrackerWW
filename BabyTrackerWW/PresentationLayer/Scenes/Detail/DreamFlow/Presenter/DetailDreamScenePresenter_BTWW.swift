...
final class DetailDreamScenePresenter_BTWW: DetailDreamScenePresenterInputProtocol_BTWW {
    ...
    init(view: DetailDreamScenePresenterOutputProtocol_BTWW,
         delegate: DetailDreamSceneDelegate_BTWW,
    ...
    private func addNewFlow() {
        dream = Dream(index: delegate.shareStateForDetailDreamScene().lifeCycle.endIndex,
                      fallAsleep: .crying,
    ...
    private func didSelectFlow(at index: Int) {
        selectedIndex = index
        dream = delegate.shareStateForDetailDreamScene().lifeCycle[index] as? Dream
    }
    ...
    func saveButtonTapped() {
        selectedIndex == nil ? delegate.add(new: dream) : delegate.change(dream)
    }
