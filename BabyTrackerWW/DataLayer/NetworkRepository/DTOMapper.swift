...
protocol DTONetworkMapperProtocol { // Как нибудь переименовать? Не оч нравится название, т.к. класс не только мапит..
    
}


final class DTONetworkMapperImpl: DTONetworkMapperProtocol {
    ...
    func request(repo: NetworkRepository, callback: @escaping (Result<[LifeCycle], Error>) -> ()) {
        do {
            let request = try self.request.create()
            let session = self.session.create()
            repo.execute(request: request, session: session) { result in
                switch result {
