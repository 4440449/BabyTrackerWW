//
//  LifeCyclesCardPersistentRepositoryTest.swift
//  BabyTrackerWWTests
//
...
class LifeCyclesCardPersistentRepositoryTest: XCTestCase {
    
    func test_synchronizeWith_failedWakeRepo_andCancelChanges() throws {
        //Given
        let error = PersistentRepositoryErrorMock.error
        ...
        let lcRepo = LifeCyclesCardPersistentRepository_BTWW(dreamRepository: dreamRepo, wakeRepository: wakeRepo)
        ...
        lcRepo.synchronize(newValue: newValue, oldValue: oldValue, date: date) { result in
            ...
            case let .failure(error): XCTAssertTrue(error is PersistentRepositoryErrorMock)
            }
